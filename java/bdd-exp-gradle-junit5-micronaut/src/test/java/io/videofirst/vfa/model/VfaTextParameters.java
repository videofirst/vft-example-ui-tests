...
public class VfaTextParameters {
    ...
    public static VfaTextParameters parse(String text, List<Object> paramValues, List<String> paramNames) {
        ...
        if (textParam.values == null || textParam.values.isEmpty() || textParam.indexes == null || textParam.indexes.isEmpty()) {
            return null;
        }
        return textParam;
    }
        ...
        if (text != null && this.indexes != null && this.values != null && !this.values.isEmpty()) {
            for (int index : this.indexes) {
                ...
                if (index < 0 || index >= paramCount) {
                    throw new VfaException("Invalid index " + index + " - must be between 0 and " + paramCount + " for text \"" + text + "\"");
                }
            }
        }
    }
